...
sh $BRONCHO_INTEGRATION_TOP/broncho_build/broncho_build_clean.sh root
...
#brn_cpdir  $BRONCHO_INTEGRATION_TARGET_PRODUCT/root $BRONCHO_INTEGRATION_TARGET_PRODUCT/root-$CUSTOMER
	...
	tar -czvf $BRONCHO_INTEGRATION_TARGET_PRODUCT/root-$CUSTOMER.tar.gz -C $BRONCHO_INTEGRATION_TARGET_PRODUCT root-$CUSTOMER 

	if [ -x /sbin/mkfs.cramfs ]; then
		/sbin/mkfs.cramfs $BRONCHO_INTEGRATION_TARGET_PRODUCT/root-$CUSTOMER $BRONCHO_INTEGRATION_TARGET_PRODUCT/root-$CUSTOMER.bin
	else
		echo 
		echo
