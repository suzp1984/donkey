...
CWD=$PWD
export PYTHON_PARSER="$PWD/python/BronchoParser.py -f $PWD/../config"
...
do
	...
	cpu_type=`$PYTHON_PARSER -g $device_item cpu_type`
	bootloader=`$PYTHON_PARSER -g $device_item bootloader`
	export out_dir=`$PYTHON_PARSER -g $device_item out_dir`
	...
	sed -e 's/@out_dir@/'"$out_dir"'/g' -e 's/@kernel@/'"$kernel_version"'/g' -e 's#@device_path@#'"$device_path"'#g' -e 's/@flash_size@/'"$flash_size"'/g' -e 's/@target_product@/'"$target_product"'/g'  -e 's/@cpu_type@/'"$cpu_type"'/g' -e 's/@bootloader@/'"$bootloader"'/g' build_env_template.sh > ../build_env.sh
