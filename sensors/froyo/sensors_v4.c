...
static int __sensors_data_open(struct sensors_data_device_t *device, native_handle_t *sensors_handle)
{
	...
	/**test property
	property_get(PROPERTY_O_SENSOR, m_status, "disabled");
	LOGE(":____+++++the property of O_snesor is %s", m_status);
	if(strcmp(m_status, "disabled") == 0) {
		LOGE(":--o_sensor is disabled");
	} */
