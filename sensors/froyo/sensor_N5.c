...
#define SWITCH_O_SENSOR "sys.sensor.orientation"  //enable, disable m_sensor
#define PROPERTY_G_SENSOR "ro.broncho.GS"
#define PROPERTY_M_SENSOR "ro.broncho.MS"
#define PROPERTY_L_SENSOR "ro.broncho.LS"
#define PROPERTY_O_SENSOR "ro.broncho.OS"
#define PROPERTY_P_SENSOR "ro.broncho.DS"
...
#define ID_LIGHT (ID_BASE+1)
#define ID_PROXIMITY (ID_BASE+2)
#define ID_ORIENTATION (ID_BASE+3)
#define ID_MAGNETIC_FIELD (ID_BASE+4)

/*support g_sensor l_sensor orintation */
#define SENSORS_SUPPORT_COUNT 4
#define SENSORS_FD_COUNT 3

#define LSG                     (980.0f)
#define G_SENSOR_CONVERT                 0.00981f
#define M_SENSOR_CONVERT		0.1f
#define L_SENSOR_CONVERT		1.0f
#define P_SENSOR_CONVERT		1.0f
...
#define P_SENSOR_NAME "p_sensor"
#define O_SENSOR_NAME "o_sensor"
//#define L_SENSOR_NAME "l_sensor"
...
#define L_SENSOR_DEVICE_STATE "/sys/class/l_sensor/l_sensor_class/als_on"
#define L_SENSOR_DEVICE_INTERRUPT "/sys/class/l_sensor/l_sensor_class/als_interrupt"
#define L_SENSOR_DEVICE_LOWT "/sys/class/l_sensor/l_sensor_class/als_lowT"
#define L_SENSOR_DEVICE_HIGHT "/sys/class/l_sensor/l_sensor_class/als_highT"

/** promxy sensor **/
#define P_SENSOR_DEVICE_STATE "sys/class/p_sensor/p_sensor_class/prox_on"
...
/*
 * maps between system property and sensors
 */
struct property_sensors_map {
	char* property;
...
static const struct sensor_t g_sensor_lis35de = {
	.name = "Lis35DE 3-axis Accelerometer",
	.vendor = "STMicroelectronics",
	.version = 1,
	.handle = ID_ACCELERATION,
...
static const struct sensor_t o_sensor_broncho = {
	.name = "Broncho compass Orientation sensor",
	.vendor = "Broncho",
	.version = 1,
	.handle = ID_ORIENTATION,
...
static const struct sensor_t m_sensor_hmc5883l = {
...
static const struct sensor_t l_sensor_taos258x = {
	.name = "TAOS 258x ambient light sensor",
	.vendor = "TAOS Inc.",
	.version = 1,
	.handle = ID_LIGHT,
...
static const struct sensor_t p_sensor_tmd2771 = {
	.name = "TMD 2771 proximity sensor",
	.vendor = "TMD 2771",
	.version = 1,
	.handle = ID_PROXIMITY,
...
static struct property_sensors_map brn_sensors[SENSORS_SUPPORT_COUNT] = {
	{
		.property = PROPERTY_G_SENSOR,
		.sensor = &g_sensor_lis35de,
		.name = "Lis35DE",
	},
	{
		.property = PROPERTY_L_SENSOR,
		.sensor = &l_sensor_taos258x,
		.name = "taos258x",
	},
	{
		.property = PROPERTY_P_SENSOR,
		.sensor = &p_sensor_tmd2771,
		.name = "tmd2771",
	},
	{
		.property = PROPERTY_O_SENSOR,
		.sensor = &o_sensor_broncho,
		.name = "xxxx",
	}, 
	
};

static struct sensor_t sensor_list[SENSORS_SUPPORT_COUNT];
/** zxsu: notice return the number of sensors **/
static int  sensors_get_list(struct sensors_module_t *module,
                                 struct sensor_t const** list)
{
	LOGE("__ in sensors_get_list!!!!");
	int sensor_count = 0;
	int i = 0;
	char status[PROPERTY_VALUE_MAX];

	for(i=0; i < SENSORS_SUPPORT_COUNT; i++) {
		property_get(brn_sensors[i].property, status, "Lis35DE");
		if(!strcmp(status, brn_sensors[i].name)) {
			sensor_list[sensor_count] = *brn_sensors[i].sensor;
			sensor_count++;
		}
	}

	/*fuck o_sensor*/
	property_get(PROPERTY_G_SENSOR, status, "Lis35DE");
	if(strcmp(status, "n")) {
		property_get(PROPERTY_O_SENSOR, status, "magic");
		if(strcmp(status, "n")) {
			sensor_list[sensor_count] = o_sensor_broncho;
			sensor_count++;
		}
	}

	LOGE("__ in sensors_get_list: the sensor count is: %d", sensor_count);
...
/************************sensor control************************/
static int control_open_input(char* dev_name, int mode)
{
	int fd = -1;
	...
	*filename++ = '/';
	...
	if (fd < 0) {
		...
		mkfifo("/data/etc/tmp_fifo", 0777);
		fd = open("/data/etc/tmp_fifo", O_NONBLOCK | O_RDONLY);
	}
	...
	for (i = 0; i < SENSORS_FD_COUNT; i++) {
		switch(i) {
			case ID_ACCELERATION:
				...
				break;
			case ID_ORIENTATION:
				fds[i] = control_open_input(O_SENSOR_NAME, O_RDONLY);
				break;
			case ID_LIGHT:
				...
				// TODO:XXX bugs here!!
			case ID_PROXIMITY:
				fds[i] = control_open_input(P_SENSOR_NAME, O_RDONLY);
				break;
			default:
				break;
		}
...
static void control_l_sensors(struct sensors_control_context_t* dev, int enabled)
{
	...
	if(enabled) 
	{
		...
		dev->active_sensors |= SENSORS_LIGHT;
	...
	close(control_state_fd);
} 

static void control_p_sensor(struct sensors_control_context_t* dev, int enabled)
{
	int control_fd = -1;
	char cmd[32];
...
static void control_enable_disable_sensors(struct sensors_control_context_t* dev, int handle, int enabled)
{
	switch(handle)
	{
		case ID_ACCELERATION:
			...
			break;
		case ID_LIGHT:
			if((dev->active_sensors & SENSORS_LIGHT) && (!enabled)) {
				//light sensor was enabled, want to disable it
				control_l_sensors(dev, enabled);
			} else if (!(dev->active_sensors & SENSORS_LIGHT) && enabled) {
				control_l_sensors(dev, enabled);
			}
			break;
		case ID_PROXIMITY:
			if((dev->active_sensors & SENSORS_PROXIMITY) && (!enabled)) {
				control_p_sensor(dev, enabled);
			} else if(!(dev->active_sensors & SENSORS_PROXIMITY) && enabled) {
				control_p_sensor(dev, enabled);
			}
			break;
		case ID_ORIENTATION:
			if(enabled) {
				property_set(SWITCH_O_SENSOR, "enabled");
			} else {
				property_set(SWITCH_O_SENSOR, "disabled");
			}
...
static sensors_data_t gen_orientation_with_msensor(sensors_data_t m_sensor_data, sensors_data_t g_sensor_data)
{
	...
	o_sensor.orientation.azimuth = (int)azimuth;
	o_sensor.orientation.pitch = (int)pitch;
	o_sensor.orientation.roll = (int)roll;
 ...
 */
static sensors_data_t gen_orientation_without_msensor(sensors_data_t m_sensor_data, sensors_data_t g_sensor_data)
{
	sensors_data_t o_sensor;
	float azimuth;
	float pitch;
...
static int handle_input_event(struct sensors_data_context_t* dev, int handle, sensors_data_t* values)
{
	...
	property_get(SWITCH_O_SENSOR, o_status, "disabled");
	...
	if(event.type == EV_ABS) {
		...
		switch(event.code) {
			...
			case ABS_THROTTLE:
				if (handle == ID_LIGHT) {
					dev->l_sensor_ready = 1;
					dev->sensors[handle].light = event.value * L_SENSOR_CONVERT;
				}
				break;
			case ABS_DISTANCE:
				if(handle == ID_PROXIMITY) {
					dev->p_sensor_ready = 1;
					dev->sensors[handle].distance = event.value * P_SENSOR_CONVERT;
				}
				break;
			default:
				break;
		...
		} else if (dev->l_sensor_ready) {
			dev->l_sensor_ready = 0;
			dev->sensors[ID_LIGHT].sensor = ID_LIGHT;
			dev->sensors[ID_LIGHT].time = t;

			*values = dev->sensors[ID_LIGHT];
			return ID_LIGHT;
		} else if (dev->p_sensor_ready) {
			dev->p_sensor_ready = 0;
			dev->sensors[ID_PROXIMITY].sensor = ID_PROXIMITY;
			dev->sensors[ID_PROXIMITY].time = t;

			*values = dev->sensors[ID_PROXIMITY];
			return ID_PROXIMITY;
		}
	}
   ...
   // int calibration_fd = -1;
    ...
    if (!strcmp(name, SENSORS_HARDWARE_CONTROL))
    {
        ...
        *device = &device_control->device.common;
		...
		device_data->gen_orientation = gen_orientation_without_msensor;
