...
/*
 *TODO: 
 *	1.add property sys.sensors.orientation
 *  2.add celibration file /data/etc/m_sensor
 *
 */
...
#define G_SENSOR_RINGBUFFER_SIZE 50
...
static void control_enable_disable_sensors(struct sensors_control_context_t* dev, int handle, int enabled)
{
	...
	switch(handle)
	{
		...
		//	property_get(PROPERTY_O_SENSOR, status, NULL);
		//	LOGE("get the property: %s \n", status);
...
static int m_sensor_calibrate(struct sensors_data_context_t* dev, sensors_data_t* m_data)
{
...
static int set_calibrate_data(struct sensors_data_context_t* dev)
{
	...
	while(token != NULL)
	{
		...
		if((isdigit(*token) || *token == '-') && i < 6) {
			...
			LOGE("!!++!!the calibration data %d is %f", i, dev->calibrate_data[i]);
			i++;
		}
...
static int __sensors_data_open(struct sensors_data_device_t *device, native_handle_t *sensors_handle)
{
	...
	/*
	 * g_sensor ringbuffer setting
	 */
	dev->g_ringbuffer = ringbuffer_create(NULL, G_SENSOR_RINGBUFFER_SIZE, NULL);
	dev->g_buffer_data = (sensors_data_t*)calloc(G_SENSOR_RINGBUFFER_SIZE, sizeof(sensors_data_t));
	/*
	...
	if(set_calibrate_data(dev) < 0) {
		...
		LOGE("!!!set the --no_calibration");
	} else {
		//set calibrate func for m_sensor
		dev->m_sensor_calibrate = m_sensor_calibrate;
		LOGE("!!!set the calibration!!!");
	}
...
static int __sensors_data_close(struct sensors_data_device_t *device)
{
	...
	ringbuffer_destroy(dev->g_ringbuffer);
	free(dev->g_buffer_data);
	free(dev->mfds);
	free(dev->sensors);
...
Ret g_sensor_sum(void* ctx, void* data)
{
	sensors_data_t* sum = (sensors_data_t*)ctx;
	sensors_data_t* g_data = (sensors_data_t*)data;
	sum->acceleration.x += g_data->acceleration.x;
	sum->acceleration.y += g_data->acceleration.y;
	sum->acceleration.z += g_data->acceleration.z;

	return RET_OK;
}

Ret buffer_size(void* ctx, void* data)
{
	int* count = (int*)ctx;
	(*count)++;
		...
		for(i = 0; i < SENSORS_FD_COUNT; i++)
		{
			if(dev->mfds[i].revents == POLLIN)
			{
				...
				if(event.type == EV_ABS) {
					...
					//TODO:XXX
					// Orintation cal
					if((dev->pending_sensors & G_M_SENSOR_ALL) && !strcmp(o_status, "enabled")) {
						dev->pending_sensors = 0;
					...
					if (dev->g_sensor_axis & SENSORS_AXIS_ALL) {
						...
						*(dev->g_buffer_data[]) = dev->sensors[i];
						ringbuffer_append(dev->g_ringbuffer, (void*)dev->sensors[i]);
						ringbuffer_foreach(dev->g_ringbuffer, buffer_size, &g_buffer_size);
						ringbuffer_foreach(dev->g_ringbuffer, g_sensor_sum, &(dev->pending_g_sensor));
						dev->pending_g_sensor.acceleration.x = dev->pending_g_sensor.acceleration.x / g_buffer_size;
						dev->pending_g_sensor.acceleration.y = dev->pending_g_sensor.acceleration.y / g_buffer_size;
						dev->pending_g_sensor.acceleration.z = dev->pending_g_sensor.acceleration.z / g_buffer_size;
        ...
        struct hw_device_t** device)
{
    ...
    if (!strcmp(name, SENSORS_HARDWARE_CONTROL))
    {
        ...
        *device = &device_control->device.common;
    	...
    	LOGE("the client programe begins...!!");
		...
		device_data->gen_orientation = gen_orientation;
