...
#define L_SENSOR_DEVICE_STATE "/sys/class/l_sensor/l_sensor_class/device_state"
...
static struct property_sensors_map brn_sensors[SENSORS_SUPPORT_COUNT] = {
	...
	{
		.property = PROPERTY_M_SENSOR,
		.sensor = &m_sensor_hmc5883l,
		.name = "HMC5883L",
	},
};
...
static void control_l_sensors(struct sensors_control_context_t* dev, int enabled)
{
	...
	if(enabled) 
	{
		als_lowT_fd = open(L_SENSOR_DEVICE_LOWT, O_WRONLY);
		if(als_lowT_fd < 0) {
			LOGE("in func %s: cannot open %s.", __FUNCTION__, L_SENSOR_DEVICE_LOWT);
			goto fail3;
		}

		als_highT_fd = open(L_SENSOR_DEVICE_HIGHT, O_WRONLY);
		if(als_highT_fd < 0) {
			LOGE("in func %s: cannot open %s.", __FUNCTION__, L_SENSOR_DEVICE_HIGHT);
			goto fail2;
		}

		als_interrupt_fd = open(L_SENSOR_DEVICE_INTERRUPT, O_WRONLY);
		if(als_interrupt_fd < 0) {
			LOGE("in func %s: cannot open %s.", __FUNCTION__, L_SENSOR_DEVICE_INTERRUPT);
			goto fail1;
		}
		...
		sprintf(cmd, "1");
		write(als_lowT_fd, cmd, sizeof(cmd));
		sprintf(cmd, "2");
		write(als_highT_fd, cmd, sizeof(cmd));
		sprintf(cmd, "1");
		write(als_interrupt_fd, cmd, sizeof(cmd));
		write(control_state_fd, cmd, sizeof(cmd));
