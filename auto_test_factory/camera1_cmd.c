...
static int camera1_cmd_execute(CmdInterface* thiz, void* ctx)
{
	DECLES_PRIV(priv, thiz);
	char content[4];
	int i = 0;
	...
	parcel_set_sub_cmd(reply, AUTOTEST_CAMERA1);
...
static void camera1_cmd_destroy(CmdInterface* thiz)
{
	SAFE_FREE(thiz);
}
...
CmdInterface* camera1_cmd_create(CmdListener* listener)
{
	...
	if (thiz != NULL) {
		DECLES_PRIV(priv, thiz);
		thiz->execute = camera1_cmd_execute;
		thiz->destroy = camera1_cmd_destroy;

		thiz->cmd = AUTOTEST_CAMERA1;
