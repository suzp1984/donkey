...
static void* receive_cmd_earpiece_run(void* ctx)
{
...
static int receive_cmd_execute(CmdInterface* thiz, void* ctx)
{
	DECLES_PRIV(priv, thiz);
	int rt = 0;
	char content[4];
	...
	parcel_set_sub_cmd(reply, AUTOTEST_RECEIVE);
	...
	priv->thread_run = 1;
	pthread_create(&t, NULL, receive_cmd_earpiece_run, (void*)thiz);
...
static void receive_cmd_destroy(CmdInterface* thiz)
{
	SAFE_FREE(thiz);
}
...
CmdInterface* receive_cmd_create(CmdListener* listener)
{
	...
	if (thiz != NULL) {
		DECLES_PRIV(priv, thiz);
		thiz->execute = receive_cmd_execute;
		thiz->destroy = receive_cmd_destroy;

		thiz->cmd = AUTOTEST_RECEIVE;
